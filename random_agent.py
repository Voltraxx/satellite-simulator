...
def train_q_learning(episodes=10):
    ...
    for episode in range(episodes):
        ...
        if (episode + 1) % 100 == 0:
            ...
            print(
                f"Episode {episode + 1}/{episodes} | Avg Reward (last 100): {avg_reward:.2f} | Epsilon: {epsilon:.3f}")
...
# >>> ADDED: Agente aleatorio (baseline uniforme sobre acciones)
def run_random_baseline(episodes=3, max_steps=100, render=False):
    """
    Ejecuta un agente aleatorio (acciones ~ Uniform(ActionSpace))
    y muestra su desempeño como línea base.
    """
    print("=" * 50)
    print("🎲 RUNNING RANDOM-AGENT BASELINE (Uniform over actions)")
    print(f"📈 Total episodes: {episodes}")
    print("=" * 50)

    render_mode = 'human' if render else None
    env = CubeSatDetumblingEnv(render_mode=render_mode, max_steps=max_steps)
    ...
    for ep in range(episodes):
        ...
        print(f"\n🎮 Random Episode {ep + 1}/{episodes}")
        print("-" * 30)

        while not done and steps < max_steps:
            action = env.action_space.sample()  # ← acción uniforme
            ...
            if steps % 20 == 0:
                ang_norm = np.linalg.norm(obs[4:7])
                print(f"  Step {steps}: Angular velocity norm = {ang_norm:.4f} rad/s")

        total_rewards.append(ep_reward)

        if terminated:
            success_count += 1
            print(f"  ✅ SUCCESS (random) en {steps} pasos")
        else:
            print(f"  ⏰ Episodio terminó a los {steps} pasos")

        print(f"  📊 Recompensa total (random): {ep_reward:.2f}")

    env.close()

    print("\n" + "=" * 50)
    print("📈 RANDOM-AGENT SUMMARY")
    print(f"🏆 Tasa de éxito: {success_count}/{episodes} ({100 * success_count / episodes:.1f}%)")
...
if __name__ == "__main__":
    ...
    # Train the agent (se deja tal cual)
    # q_table, bins = train_q_learning(episodes=3)

    # Evaluate the trained agent (se deja tal cual)
    # evaluate_q_learning(q_table, bins, episodes=3)

    # >>> ADDED: ejecutar baseline aleatorio para comparar
    run_random_baseline(episodes=3, max_steps=100, render=False)
